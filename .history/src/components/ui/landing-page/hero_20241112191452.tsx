  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      type: 'ai',
      content: "Welcome! I'm your AI property assistant. How can I help you today?",
      icon: <Building2 className="h-4 w-4" />
    },
    {
      id: 2,
      type: 'ai',
      content: "You can enter any property address, and I'll analyze its value using advanced AI.",
      icon: <MapPin className="h-4 w-4" />
    },
    {
      id: 3,
      type: 'user',
      content: "1234 Innovation Drive, Silicon Valley, CA 94025",
      icon: <Search className="h-4 w-4" />
    },
    {
      id: 4,
      type: 'ai',
      content: "Processing your request... Analyzing property details and market data.",
      icon: <div className="h-4 w-4 rounded-full border-2 border-primary border-r-transparent animate-spin" />
    }
  ]);
  ...
  useEffect(() => {
    if (animationStage === 'typing') {
      const chatSequence = async () => {
        ...
        // First message (slightly delayed start)
        await new Promise(resolve => setTimeout(resolve, 1000));
        setChatStep(1); // "Hello! Please enter..."
        
        // Second message appears (increased from 2000)
        await new Promise(resolve => setTimeout(resolve, 2500));
        setChatStep(2); // "For example: 1234..."
        
        // User input appears (increased from 2000)
        await new Promise(resolve => setTimeout(resolve, 2500));
        setChatStep(3); // User typing effect
        
        // Processing message (increased from 2000)
