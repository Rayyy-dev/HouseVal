      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        <div ref={demoRef} className="relative h-[580px] rounded-3xl overflow-hidden bg-gradient-to-br from-background via-background/95 to-muted/90 shadow-[0_0_40px_-15px] shadow-primary/20 border border-border/50 backdrop-blur-xl">
          <div className="demo-container">
            ...
            {/* Address Search Animation - Chat-like typing stage */}
              ...
              <div className="demo-address absolute inset-0 flex items-center justify-center p-6">
                <div className="w-full max-w-2xl space-y-4">
                  ...
                  <div className="flex items-start gap-3 animate-fade-in">
                    ...
                    <div className="flex-1">
                      <div className="bg-muted/50 backdrop-blur-sm rounded-2xl rounded-tl-none p-4 text-sm text-foreground/80 shadow-sm border border-border/50">
                        Please enter a property address to begin the AI analysis...
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 animate-fade-in-delay-1">
                    ...
                    <div className="flex-1">
                      <div className="bg-primary/10 backdrop-blur-sm rounded-2xl rounded-tl-none p-4 text-sm shadow-sm border border-primary/20">
                  ...
                  <div className="flex items-start gap-3 animate-fade-in-delay-2">
                    ...
                    <div className="flex-1">
                      <div className="bg-muted/50 backdrop-blur-sm rounded-2xl rounded-tl-none p-4 space-y-3 shadow-sm border border-border/50">
                        <p className="text-sm text-foreground/80">
                          Processing your request...
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {[
                            { icon: Building2, text: "Analyzing Property" },
                            { icon: MapPin, text: "Verifying Location" },
                            { icon: DollarSign, text: "Checking Market Data" }
                          ].map((feature, index) => (
                            <div 
                              key={index} 
                              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-background/50 border border-border/50"
                    <p className="text-sm text-muted-foreground">
                      AI Assistant ready to analyze property details...
                    </p>
                  </div>

                  {/* Feature Tags */}
                  <div className="flex flex-wrap gap-2 animate-fade-in-delay-2">
                    {[
                      { icon: Building2, text: "Instant Valuation" },
                      { icon: MapPin, text: "Location Analysis" },
                      { icon: DollarSign, text: "Price Trends" }
                    ].map((feature, index) => (
                      <div key={index} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/5 border border-primary/10">
                        <feature.icon className="h-3.5 w-3.5 text-primary" />
                        <span className="text-xs font-medium text-primary">{feature.text}</span>
                      </div>
                    ))}
                  </div>
                </div>
