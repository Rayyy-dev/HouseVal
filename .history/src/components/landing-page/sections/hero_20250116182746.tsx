...
const TypewriterText = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (currentIndex <= promptText.length) {
        setText(promptText.slice(0, currentIndex));
        currentIndex++;
      } else {
  ...
  return (
    <div className="font-mono text-base text-muted-foreground">
      {text}
      <span className="inline-block w-2 h-5 bg-primary/80 animate-pulse ml-1">_</span>
    </div>
  );
};
          ...
          <div className="demo-container">
            ...
            {animationStage === 'typing' && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="p-6 bg-black/90 rounded-lg border border-border/10"
              >
